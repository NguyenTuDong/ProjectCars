<template>
  <tr>
    ...
    <td>
      <div :title="convenient.count + ' / ' + carCountApprove">
        <div v-if="convenient.count > 0" :style="{ width: percent, opacity: opacity }" class="records-ratio"></div>
        <span>{{ percent }}</span>
      </div>
    </td>
...
export default {
  ...
  computed: {
    percent(){
      var self = this;
      return Math.round(((self.convenient.count / self.carCountApprove) * 100 + Number.EPSILON) * 100) / 100 + '%';
    },
    opacity(){
      var self = this;
  ...
  methods: {
    ...
    showPopup() {
      this.$emit('showPopup', this.convenient);
    },
    cancle() {
